  ...
  const fetchAppointmentDetails = useCallback(async () => {
    try {
      ...
      if (appointment.doctor && appointment.doctor.name) {
        ...
        const doctorName = appointment.doctor.name.startsWith('Dr.') ?
          appointment.doctor.name : `Dr. ${appointment.doctor.name}`;
        document.title = `Appointment with ${doctorName} on ${formattedDate} | MedConnect`;
      } else if (appointment.doctor && appointment.doctor.user && appointment.doctor.user.name) {
        const formattedDate = new Date(appointment.appointmentDate).toLocaleDateString();
        const doctorName = appointment.doctor.user.name.startsWith('Dr.') ?
          appointment.doctor.user.name : `Dr. ${appointment.doctor.user.name}`;
        document.title = `Appointment with ${doctorName} on ${formattedDate} | MedConnect`;
      } else {
                ...
                <Col md={6}>
                  ...
                  <p className="mb-3">
                    <strong>Fee:</strong> {
                      appointment.fees
                        ? `₹${appointment.fees}`
                        : (appointment.doctor && appointment.doctor.fees)
