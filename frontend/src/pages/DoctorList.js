...
const DoctorList = () => {
  ...
  // Add this function to format timings in a consistent way
  const formatDoctorTimings = (timings, availableDays) => {
    ...
    if (typeof timings[0] === 'object') {
      // Take the first available timing for display in the card
      const firstAvailableDay = timings.find(t => t.isAvailable);
    ...
    if (typeof timings[0] === 'string' && timings.length >= 2) {
      return `Hours: ${timings[0]} - ${timings[1]}`;
    }
          ...
          <Row>
            {doctors && doctors.map((doctor) => (
              <Col key={doctor._id} sm={12} md={6} lg={4} xl={3} className="mb-4">
                <DoctorCard doctor={doctor} formatTimings={formatDoctorTimings} />
              </Col>
            ))}
          </Row>
        </>
